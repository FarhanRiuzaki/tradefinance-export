                                       ...
                                       <?php $no = 0; foreach($record->transactions_details as $key => $val): $no++; ?>
                                            <tr>
                                                ...
                                                <td class='text-center'>
                                                    ...
                                                    <button class='btn btn-danger delFile btn-icon'><span class='fa fa-times'></span></button>
                                                </td>
                                            </tr>
                                       <?php endforeach;?>
                                    </tbody>
    ...
    <script>
        ...
        $('.addFile').on('click', function () {  
            ...
            html = "<tr>"
                ...
                + "<td><input type='text' data-name='note' name='transactions_details["+count+"][no_rek]' class='form-control discrepancy'></input></td>"

                + "<td width='12%'><select name='transactions_details["+count+"][d_c]' class='form-control'><option value='Debet'>Debet</option><option value='Kredit'>Kredit</option></select></td>"

                + "<td width='12%'><select name='transactions_details["+count+"][currency]' class='form-control'><option value='IDR'>IDR</option><option value='USD'>USD</option><option value='AUD'>AUD</option><option value='SGD'>SGD</option><option value='NZD'>NZD</option><option value='EUR'>EUR</option><option value='GBP'>GBP</option><option value='JPY'>JPY</option><option value='CHF'>CHF</option><option value='CNY'>CNY</option></select></td>"

                + "<td>"+ select +"</td>"
                + "<td><input type='text' data-name='note' name='transactions_details["+count+"][cost_center]' class='form-control discrepancy'></input></td>"
                + "<td><input type='text' data-name='note' name='transactions_details["+count+"][reff_no]' class='form-control discrepancy'></input></td>"
                + "<td><input type='text' data-name='note' name='transactions_details["+count+"][note]' class='form-control discrepancy'></input></td>"
                + "<td class='text-center'><button class='btn btn-danger delFile btn-icon'><span class='fa fa-times'></span></button></td>"
                + "</tr>";
