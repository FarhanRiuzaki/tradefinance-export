        ...
        <div class="kt-portlet">
			...
			<?= $this->Form->create($uploadDoc,['class'=>'kt-form','type'=>'file', 'id' => 'form-submit']) ?>
				<div class="kt-portlet__body">
                    ...
                    <!-- set status 0 -->
                    <input type="hidden" name='status_sor' value='0'>
...
<?php $this->start('script');?>
    <script>
        ...
        $('.addFile').on('click', function () {  
            ...
            select  = "<select name='upload_doc_file["+ count +"][file_name]' class='form-control doc'>";
            $.each(param, function (k, v) {  
                select += "<option value='" + v.doc_code + "'>" + v.doc_code + ' - ' + v.doc_desc + "</option>";
            });
            select  += "</select>";
            ...
            html = "<tr>"
                + "<td>"+ no +"</td>"
                + "<td>" + select + "</td>"
                + "<td><input data-name='note' name='upload_doc_file["+count+"][note]' class='form-control'><input data-name='no_sor' type='hidden' name='upload_doc_file["+count+"][no_sor]' class='form-control' value='"+ no_sor +"'></td>"
