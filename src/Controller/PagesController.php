...
class PagesController extends AppController
{
    ...
    function beforeFilter(\Cake\Event\Event $event){
        ...
        if(isset($this->Security) && $this->action = 'index' ){
            $this->Security->config('unlockedFields',['UserGroups']);
            $this->Security->config('unlockedFields',['index']);
            ...
            if ($this->request->is('post')) {
                ...
                if(!empty($checkUser)){
                    ...
                    if($hasher->check($password,$checkUser->password)){
                        ...
                        // LOGIN TIME UPDATE
                        // $articlesTable  = TableRegistry::getTableLocator()->get('Users');
                        // $article        = $articlesTable->get($checkUser->id); // Return article with id 12
                        // $article->login_time = date("Y-m-d H:i:s");
                        // $articlesTable->save($article);

                        // $conn = ConnectionManager::get('default');
                        // $conn->execute('UPDATE users SET login_time = DATE_ADD(NOW(), INTERVAL 7 HOUR) WHERE id = '.$checkUser->id.'');
    ...
    {   
        ...
        if(!empty($this->userData)){
            ...
            $conn = ConnectionManager::get('default');
            $conn->execute('UPDATE users SET logout_time = DATE_ADD(NOW(), INTERVAL 7 HOUR) WHERE id = '.$this->userData->id.'');
